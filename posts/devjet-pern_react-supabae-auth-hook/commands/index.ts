...
module.exports = {
    ...
    run: async (toolbox: GluegunToolbox) => {
        ...
        const isAtRoot = await toolbox.prompt.confirm('Please confirm that you are at the root folder of your project (a small help, is the folder that contains the .git folder!)')
        if (!isAtRoot) return toolbox.print.error("Auchh, we cant resolve that yet, please move to the root folder and run this plugin again!")
        const isReady = await toolbox.prompt.confirm('We strongly recommend to run devjet generators on a new branch or with a previous commit. Are you ready to continue?')
        if (!isReady) return toolbox.print.error("Heyy, take your time, no problem!")

        toolbox.print.info("Hey, if you changed the default folder structure, some things may break, just a small disclosure...")
        ...
        if (stack === "react") {
            ...
            await toolbox.patching.append('.env.example', 'REACT_APP_SUPABASE_URL=\nREACT_APP_SUPABASE_API_KEY=\n')
            ...
            await toolbox.patching.patch('./src/index.tsx', { insert: 'import { AuthProvider } from "utils/authHook";', after: 'import chakraTheme from "utils/chakraTheme";' })
            await toolbox.patching.patch('./src/index.tsx', { insert: '<AuthProvider>', after: '<Provider store={store}>' })
            await toolbox.patching.patch('./src/index.tsx', { insert: '</AuthProvider>', before: '</AuthProvider>' })
        ...
        } else toolbox.print.error("Sorry, we don't have support for that one yet!")
    },
};
